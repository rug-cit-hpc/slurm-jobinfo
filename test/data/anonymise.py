...
CLUSTER_PREFIX_ANON = 'mycluster'
# All usernames will be renamed to <USERNAME_PREFIX_ANON><number>.
USERNAME_PREFIX_ANON = 'user'

# Column number (start counting at 0) of sacct output file that contains the usernames
...
def find_cluster_prefixes():
    ...
    # Find all nodenames.
    nodes = re.findall('NodeName=(\S+)', scontrol)
    # Find the prefix by splitting by the hypen symbol.
    prefixes = [node.split('-', 1)[0] for node in nodes]
...
def generate_anonymised_usernames(usernames):
    ...
    return {user: USERNAME_PREFIX_ANON + str(i+1) for user, i in zip(usernames, range(len(usernames)))}
...
def generate_anonymised_cluster_prefixes(cluster_prefixes):
    ...
    return {prefix: CLUSTER_PREFIX_ANON + str(i+1) for prefix, i in zip(cluster_prefixes, range(len(cluster_prefixes)))}
...
def anonymise(file, usernames, cluster_prefixes):
    ...
    for cluster_prefix, cluster_prefix_anon in generate_anonymised_cluster_prefixes(cluster_prefixes).items():
        contents = contents.replace(cluster_prefix + '-', CLUSTER_PREFIX_ANON + '-')
    ...
    if file == SCONTROL_FILE:
        contents = '\n'.join([' '.join(groups) for groups in re.findall('(NodeName=\S+) .* (CfgTRES=\S+) .*', contents)])
...
def main():
    ...
    for file in [SACCT_FILE, SSTAT_FILE, SCONTROL_FILE]:
