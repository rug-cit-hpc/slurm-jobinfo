...
def generate_anonymised_usernames(usernames):
    ...
    return {user: USERNAME_PREFIX_ANON + bytes(i+1) for user, i in zip(usernames, range(len(usernames)))}
...
def generate_anonymised_cluster_prefixes(cluster_prefixes):
    ...
    return {prefix: CLUSTER_PREFIX_ANON + bytes(i+1) for prefix, i in zip(cluster_prefixes, range(len(cluster_prefixes)))}
